...
const char * const BITCOIN_CONF_FILENAME = "astercoin.conf";
const char * const BITCOIN_PID_FILENAME = "astercoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "astercoin" is a composite category enabling all Astercoin-related debug output
            if(ptrCategory->count(string("astercoin"))) {
...
#else
    const char* pszModule = "astercoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AstercoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AstercoinCore
    // Mac: ~/Library/Application Support/AstercoinCore
    // Unix: ~/.astercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AstercoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AstercoinCore";
#else
    // Unix
    return pathRet / ".astercoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty astercoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override astercoin.conf
