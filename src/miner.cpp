...
// ***TODO*** ScanHash is not yet used in Astercoin
//
...
{
    LogPrintf("AstercoinMiner -- started\n");
    ...
    try {
        ...
        while (true) {
            ...
            */
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("AstercoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("AstercoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("AstercoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("AstercoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}
